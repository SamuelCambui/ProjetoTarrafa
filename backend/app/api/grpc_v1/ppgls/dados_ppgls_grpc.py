...
class DadosPPGLSServicer(ppgls_pb2_grpc.DadosPosGraduacaoLSServicer):
    ...
    def GetCurso(self, request: PPGLSRequest, context):
        curso = tasks_cursos_ppgls.tarefa_retorna_curso.delay(id=request.id, id_ies=request.id_ies).get()
        return PPGLSResponse(item=[curso])
    
    @cache_grpc_ppgls()
    def GetCursos(self, request, context):
        cursos = tasks_cursos_ppgls.tarefa_lista_cursos.delay(id_ies=request.id_ies).get()
        return PPGLSResponse(item=[cursos])
    
    @cache_grpc_ppgls()
    def GetDisciplina(self, request: PPGLSRequest, context):
        disciplina = tasks_disciplinas_ppgls.tarefa_lista_dados_disciplina.delay(id_disciplina=request.id_disciplina, id_ies=request.id_ies ).get()
        return PPGLSResponse(item=[disciplina])

    @cache_grpc_ppgls()    
    def GetDisciplinas(self, request: PPGLSRequest, context):
        disciplinas = tasks_disciplinas_ppgls.tarefa_lista_disciplinas_por_curso.delay(id_grade=request.id_grade, id_curso=request.id_curso,id_ies=request.id_ies).get()
