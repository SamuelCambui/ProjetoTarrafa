...
class IndicadoresPPGLSServicer(ppgls_pb2_grpc.IndicadoresPosGraduacaoLSServicer):
    ...
    def GetAbaIndicadoresCurso(self, request: PPGLSRequest, context):
        ...
        #tasks_cursos_ppgls.tarefa_media_idades_por_ano.s(id=request.id, anoi=request.anoi, anof=request.anof)
        #tasks_cursos_ppgls.tarefa_quant_alunos_e_alunas_por_semestre(id=request.id, anoi=request.anoi, anof=request.anof)
    ...
    def GetAbaRegressos(self, request: PPGLSRequest, context):
        consultas = []
        consultas.append(tasks_cursos_ppgls.tarefa_quant_alunos_vieram_gradu_e_nao_vieram_por_curso.s(id=request.id))
        ...
        job = group(consultas)
        print("EXECUTANDO GetAbaRegressos")
        result = job.apply_async()
        ret_values = result.get()  # Ajuste o timeout conforme necessário
        # try:
        # except Exception as e:
        #     print(f"Erro ao obter resultados das tarefas: {e}")
        #     result = []  # Ou lidar com o erro de maneira adequada

        response = [item for item in ret_values]
