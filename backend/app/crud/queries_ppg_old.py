from backend.app.db.db import DBConnector
...
class QueriesPPG():
    ...
    def retorna_informacao_ppg(self, id: str, db: DBConnector):
        ...
        if row:
            ...
            return {'info': texto, 'nome':row['nome'], 'nota': row['conceito'], 'siglas': row['sigla_curso'], 'url': row['contato_url'], 'email':row['contato_email']}
        return None
    ...
    def retorna_lista_ies(self, db: DBConnector):
        """   
        Retorna todas as ies cadastradas
        """
    ...
    def retorna_contagem_de_qualis_do_lattes(self, id: str, anof: int, db: DBConnector):
        """
    ...
    def retorna_contagem_de_qualis_do_lattes_anonimo(self, id: str, anof: int, db: DBConnector):
        """
        Retornar o quantitativo de artigos com qualis considerando a base de dados lattes
        ...
        row = db.fetch_all(query_artigos, id=id, anof=anof, anocoleta=anof-1)
    ...
    def retorna_contagem_de_qualis_com_listanegra(self, id: str, anoi: int, anof: int, listanegra: List, db: DBConnector):
        """
    ...
    def retorna_contagem_de_indprodart_com_listanegra(self, id: str, anoi: int, anof: int, listanegra: List, db: DBConnector):
        """
	            ...
	            with query_ids as (
                    ...
                    select ld.num_identificador, 
                        ...
                        from (select CONCAT(SUBSTRING(d."DETALHAMENTO-DO-ARTIGO"->>'@ISSN', 1, 4), '-', SUBSTRING(d."DETALHAMENTO-DO-ARTIGO"->>'@ISSN', 5)) as issn, 
                                ...
                                where lpb.tipo = 'ARTIGO-PUBLICADO' and 
                                    d."DETALHAMENTO-DO-ARTIGO"->>'@ISSN' != '' and 									  
                                    cast(d."DADOS-BASICOS-DO-ARTIGO"->>'@ANO-DO-ARTIGO' as integer) >= %(anoi)s  and
							        cast(d."DADOS-BASICOS-DO-ARTIGO"->>'@ANO-DO-ARTIGO' as integer) <= %(anof)s
                            ) as artigos_total
                        inner join lattes_docentes as ld on (ld.num_identificador = artigos_total.num_identificador)
                        group by ld.num_identificador, issn, titulo, qualis, ano
