...
class QueriesPPG():
    ...
    def retorna_anos(self, id: str, db: DBConnector):
        """
    ...
    def retorna_informacao_ppg(self, id: str, db: DBConnector):
        """        
    ...
    # def getTotalPeriodicPapersByYear(self, db: DBConnector):
    #     """
    #     Returns Total Periodic Papers by Year

    #     Parameters:
    #         db: DataBase
    #     Returns:
    #         producoes(list of dictionarys): 
    #             quantidade(key): Amount of Productions
    #             ano(key): Year
    #     """
    #     query = f"""select count(*) as quantidade, ano from (
    #                 select pa.id_producao, p.ano_publicacao as ano from producoes_autores as pa
    #                     inner join docentes_programas as dp on (dp.id_docente = pa.id_autor)
    #                     inner join producoes as p on (pa.id_producao = p.id)
    #                 where p.subtipo ilike '%em PERI%' and dp.categoria ilike '%PERMANENTE%' and pa.tp_autor ilike 'Docente'
    #                 group by pa.id_producao, p.ano_publicacao
    #                 order by p.ano_publicacao) as producoes
    #                 group by ano"""
    #     row = db.fetch_all(query)
    #     producoes = [dict(r) for r in row]
    #     return producoes

    def retorna_lista_programas(self, id_ies: str, db: DBConnector):
        """   
    ...
    def retorna_lista_ies(self, db: DBConnector):
        """   
        Retorna todas as ies do banco de dados
        """
        query = """select id_ies, nome, sigla from instituicoes order by sigla"""
        row = db.fetch_all(query)
        ies = [dict(r) for r in row]
        return ies

    def retorna_lista_curriculos_por_ies(self, id_ies: str, ano: int, db: DBConnector):
        """   
        Retorna os id_lattes a partir de um id_ies ou 'em_lote' para todas as IES cadastradas.
        Se diferencia de 'retorna_lista_curriculos_baixados_por_ies'
        """
        if id_ies == 'em_lote':
            q = f'''select p.lattes from coleta_docentes as dp 
                            inner join pessoas as p on (p.id_pessoa = dp.id_pessoa)
                            inner join programas as prog on (prog.id_programa = dp.id_programa and prog.id_ies in (
                                    select distinct id_ies from usuarios
                            ))
                            where dp.ano = {ano} and p.lattes <> ''
                            group by p.lattes, p.nome_pessoa
                            order by p.nome_pessoa'''
        
        else:
            q = f'''select p.lattes from coleta_docentes as dp 
                            inner join pessoas as p on (p.id_pessoa = dp.id_pessoa)
                            inner join programas as prog on (prog.id_programa = dp.id_programa and prog.id_ies ilike '{id_ies}')
                            where dp.ano = {ano} and p.lattes <> ''
                            group by p.lattes, p.nome_pessoa
                            order by p.nome_pessoa'''
    ...
    def retorna_lista_curriculos_baixados_por_ies(self, id_ies:str, ano:int, db: DBConnector):
        """   
        Retorna os id_lattes já baixados a partir de um id_ies ou 'em_lote' para todas as IES cadastradas
        """
        if id_ies == 'em_lote':
            q = f'''
                with q as (
                select distinct p.lattes from pessoas as p 
                inner join programas as prog on prog.id_ies in (
                    select distinct id_ies from usuarios
                )
                inner join coleta_docentes as cdo on cdo.id_programa = prog.id_programa
                where cdo.ano = {ano} and p.id_pessoa = cdo.id_pessoa
                )

                select cl.idlattes, cl.cpf, cl.linkavatar, cl.curriculo from curriculos_lattes as cl
                inner join q on q.lattes = cl.idlattes'''
        else:
            q = f'''with q as (
                    select distinct p.lattes from pessoas as p 
                    inner join programas as prog on prog.id_ies ilike '{id_ies}'
                    inner join coleta_docentes as cdo on cdo.id_programa = prog.id_programa
                    where cdo.ano = {ano} and p.id_pessoa = cdo.id_pessoa
                    )
        ...
        rows = db.fetch_all(q)#'select * from public.curriculos_lattes')
        return rows

    def retorna_lista_ies_cadastradas(self, db: DBConnector):
        """   
        Retorna todas as ies cadastradas (que possuem usuários)
        """
        query = """select distinct u.id_ies, i.nome, i.sigla from usuarios as u
                    inner join instituicoes as i on i.id_ies = u.id_ies order by i.sigla"""
        row = db.fetch_all(query)
        ies = [dict(r) for r in row]
        return ies

    def retorna_quantidade_de_artigos_em_periodicos_por_ano(self, id_ies: str, db: DBConnector):
        """         
        Retorna a quantidade de artigos em periódicos publicados por anos da instituição
        
        Paramêtros:
            id_ies: Id da instituição
            db(class): DataBase
        """
        query = """with qq as (select distinct nome_producao, ano from coleta_producoes as cp
                    inner join programas as p on p.id_programa = cp.id_programa and p.id_ies = %(id)s
                    where id_sub_tipo_producao = 18
                    order by nome_producao)
                    select count(nome_producao)  as quantidade, ano from qq
                    group by ano
                    order by ano"""
        row = db.fetch_all(query, id=id_ies)
        producoes = [dict(r) for r in row]
    ...
    def retorna_lista_de_permanentes_do_ppg(self, id: str, ano: int, db: DBConnector):
        """
    ...
    def retorna_lista_docentes_do_ppg_do_ultimo_ano(self, id: str, db: DBConnector):
        """
        Retorna a lista de todos os docentes do ultimo ano de coleta
        """

        ano =  self.retorna_ultimo_ano_coleta(db)
    ...
    def retorna_ultimo_ano_coleta(self, db:DBConnector):
        """
    ...
    def retorna_contagem_de_qualis_do_lattes(self, id: str, anof: int, anocoleta: int, db: DBConnector):
        """
    ...
    def retorna_lista_de_artigos_da_universidade(self, id: str, ano: int, db: DBConnector):
        """
        ...
        query = """
			...
			select qids.ano, qids.issn, lower(qids.titulo) as nome_producao, qids.doi, STRING_AGG(DISTINCT prog.nome, ',') as programas  from query_ids as qids
                    ...
                    inner join programas as prog on prog.id_programa = dp.id_programa and prog.id_ies = %(id)s
					group by qids.ano, qids.issn, qids.doi, nome_producao
					order by nome_producao
        """


        anocoleta = self.retorna_ultimo_ano_coleta(db)

        row = db.fetch_all(query, id=id, ano=ano, anocoleta=anocoleta)
    ...
    def retorna_contagem_de_qualis_do_lattes_anonimo(self, id: str, anof: int, ano_coleta: int, db: DBConnector):
        """
        Retornar o quantitativo de artigos com qualis considerando a base de dados lattes
    ...
    def retorna_dados_posicoes(self, id: str, anoi: int, anof: int, db: DBConnector):
        indprods, maior, menor  = self.retorna_ranking_ppgs(id, anoi, anof, db)
        media = self.retorna_indprod_medio_entre_ppgs_dada_nota(id, anoi, anof, None, db)
        media_maior = [0.0]#*(6-int(indprods[0]['nota']))
        if len(indprods) > 0:
            ncount = 0
            if indprods[0]['nota'] != 'A' and int(indprods[0]['nota']) < 7:
                #while ncount < (6-int(indprods[0]['nota'])): #considera o ranking até os nota 6
                media_maior[ncount] = self.retorna_indprod_medio_entre_ppgs_dada_nota(id, anoi, anof, str(int(indprods[0]['nota'])+(ncount+1)), db)
                    #ncount+=1
        return {'media':media, 'media_maior':media_maior, 'maior_indprod': maior, 'menor_indprod':menor, 'indprods':indprods}

    def retorna_levantemento_externos_em_bancas(self, id: str, anoi: int, anof: int, db: DBConnector):
    ...
    # def retorna_lista_de_artigos_da_universidade(self, id: str, ano: int, db: DBConnector):
    #     """
    #     Retornar o quantitativo de artigos com qualis para toda a universidade
    ...
    #         produtos(dicionario)
    #     """
    #     query_artigos = """select cp.nome_producao, cp.ano, substring(cdp.valor,2,9) as issn
	# 			from coleta_detalhamento_producao as cdp 
	# 			inner join coleta_producoes as cp on (cdp.id_producao = cp.id_producao and cp.ano = %(ano)s and cp.id_sub_tipo_producao = 18)
	# 			inner join programas as p on p.id_programa = cp.id_programa and p.id_ies = %(id)s
	...
	# 			group by cp.nome_producao, cp.ano, issn
	# 			order by cp.nome_producao ASC"""
        

    #     row = db.fetch_all(query_artigos, id=id, ano=ano)
    #     artigos = [dict(r) for r in row]
    ...
    #     products_atual, artigos_lattes = self.retorna_contagem_de_qualis_do_lattes_anonimo(id, ultimo_ano_coleta, db)

    #     artigos.extend(artigos_lattes)


    def retorna_contagem_de_qualis_com_listanegra(self, id: str, anoi: int, anof: int, listanegra: List):
        """
    ...
    def retorna_contagem_de_qualis_discentes(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_lista_de_programas_correlatos(self, id: str, db: DBConnector):
        query = """with ppg as (
    ...
    def retorna_estatisticas_de_artigos_ppgs_correlatos(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        ppgs = self.retorna_lista_de_programas_correlatos(id,db)

        estatisticas = []

        for ppg in ppgs:
            ret = self.retorna_estatisticas_de_artigos(ppg, anoi, anof, db)
    ...
    def retorna_estatisticas_de_artigos(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_contagem_de_indprodart_com_listanegra(self, id: str, anoi: int, anof: int, listanegra: List):
        """
    ...
    def retorna_contagem_de_indprodart_extrato_superior_com_listanegra(self, id: str, anoi: int, anof: int, listanegra: List, db: DBConnector):
        """
        ...
        if area:
            ...
            indprods_ext = self.retorna_indprods_extsup_medios_extratificados(id, anoi, anof, db)
    ...
    def retorna_contagem_de_indprodart_absoluto(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        products_atual = self.retorna_contagem_de_qualis_do_lattes(id, ultimo_ano_coleta, db)
    ...
    def retorna_indprods_medios_extratificados(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_indprods_extsup_medios_extratificados(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_indprods_medios_extratificados_sem_dps(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_indori(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_inddistori(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_indaut(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_inddis(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_partdis(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_indcoautoria(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_indori_medio(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_partdis_medio(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_inddistori_medio(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_indaut_medio(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retotrna_inddis_medio(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    def retorna_indcoautoria_medio(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
    ...
    # def retorna_indprod(self, id: str, anoi: int, anof: int, db: DBConnector):
    #     """
    #     Return IndProd of PPG by Year
    ...
    #     Results:
    #         indprods(list of dictionarys):
    #             indprod(float): Indprod of PPG
    #             ano(int): Year
    #     """
    #     #query = """select cast(dados->>'indProd' as float) as indprod, ano from programas_historico where id = %(id)s
	# 	#			and (ano>= %(anoi)s and ano <= %(anof)s) and id= %(id)s order by ano"""
        
    #     query = """select cast(dados->>'indProd' as float) as indProd, ano from programas_historico where id_programa = (select id_programa from programas where codigo_programa = %(id)s)
	# 				and (ano >= %(anoi)s and ano <= %(anof)s) order by ano"""
    #     row = db.fetch_all(query, id=id, anof=anof, anoi=anoi)
    #     indprods = [dict(r) for r in row]
    ...
    #         ano_in = False
    #         for i in indprods:
    #             if i['indprod'] is None:
    #                 i['indprod'] = 0.0
    #             if a == i['ano']:
    #                 ano_in = True
    #         if ano_in == False:
    #             indprods.append({'indprod': 0.0, 'ano': a})

    #     # adiciona resultado do ano atual com os dados do lattes, se ano_atual maior que o anof
    #     # Isso é necessário para que adicionar a última coluna do gráfico exceto quando o ano atual já tiver sido enviado para a Sucupira 
    #     ano_atual = self.retorna_ultimo_ano_coleta(db) + 1
    #     if anof < ano_atual:
    #         products_atual = self.retorna_quantidade_artigos_periodicos_publicados(id, ano_atual, db)
    #         permanentes = self.getNumPermanentesForPPG(id, anof, db)

    #         if permanentes and permanentes > 0:
    #             if products_atual and len(products_atual) > 0:
    #                 indProdArt = 1*products_atual[0]['A1'] + 0.875*products_atual[0]['A2'] + 0.75*products_atual[0]['A3'] + 0.625*products_atual[0]['A4'] + \
    #                     0.5*products_atual[0]['B1'] + 0.375*products_atual[0]['B2'] + \
    #                     0.25*products_atual[0]['B3'] + \
    #                     0.125*products_atual[0]['B4']
    #                 glosa = 0.25*products_atual[0]['B3'] + \
    #                     0.125*products_atual[0]['B4']
    #                 if glosa > (0.2 * indProdArt):
    #                     indProdArt = 1*products_atual[0]['A1'] + 0.875*products_atual[0]['A2'] + 0.75*products_atual[0]['A3'] + 0.625 * \
    #                         products_atual[0]['A4'] + 0.5*products_atual[0]['B1'] + \
    #                         0.375*products_atual[0]['B2'] + (0.2 * indProdArt)

    #                 indprods.append(
    #                     {'indprod': indProdArt/permanentes, 'ano': ano_atual})

    #     indprods = sorted(indprods, key=lambda k: k['ano'])
    #     return indprods

    def retorna_indprod_medio_entre_ppgs_dada_nota(self, id: str, anoi: int, anof: int, nota: int | None, db: DBConnector):
        """
    ...
    def retorna_ranking_ppgs(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        Paramêtros:
            ...
            db(class): DataBase
        """
        # query = """select avg(cast(pb.dados->>'indProd' as float)) as indprod, 
		# 			p.nome,
		# 			p.id,
		# 			i.sigla,
        #             p.nota,
		# 			m.nome as municipio,
		# 			e.uf,
		# 			pd.dados->>'status' as status,
		# 			m.latitude,
		# 			m.longitude
        #             from programas as p
        #             inner join programas_historico as pb on (p.id=pb.id) 
		# 			inner join instituicoes_programas as ip on (p.id=ip.id_programa)
		# 			inner join instituicoes as i on (i.id=ip.id_instituicao)
		# 			inner join programas_detalhes as pd on (pd.id_programa = p.id)
		# 			inner join municipios as m on (m.nome ilike pd.dados->>'municipio')
		# 			inner join estados as e on (m.codigo_uf = e.codigo_uf)
		# 			where
		# 			pd.dados->>'uf' = e.uf and
		# 			pd.dados->>'emrede' not ilike 'sim' and
        #             p.situacao ilike '%%FUNCIONAMENTO%%' and 
		# 			pb.ano >= %(anoi)s and pb.ano <= %(anof)s and
		# 			p.nota = (select nota from programas where id = %(id)s)  and
		# 			p.modalidade = (select modalidade from programas where id = %(id)s) and
		# 			p.nm_area_avaliacao = (select nm_area_avaliacao from programas where id = %(id)s) 
		# 			group by p.nome, p.id, i.sigla, p.nota, m.nome,
		# 			e.uf,
		# 			pd.dados->>'status',
		# 			m.latitude,
		# 			m.longitude, m.latitude, m.longitude
    ...
    def retorna_tempo_de_atualizacao_do_lattes(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        Paramêtros:
            ...
            db(class): DataBase
        """
        # query = """select dt_atualizacao, num_identificador, nm_completo from lattes_docentes as ld
        #             inner join (select cast(id_docente as text) from docentes_programas where id_programa = %(id)s
        #             and ano >= %(anoi)s and ano <=%(anof)s) as ids
        #             on (ld.id_sucupira like '%%'||ids.id_docente||'%%')
        #             group by dt_atualizacao, num_identificador, nm_completo"""
    ...
    async def retorna_tempo_de_atualizacao_do_lattes_do_professor(self, id: str, db: DBConnector):
        """
    ...
    async def retorna_qualis_das_producoes(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        Retorna a quantidade de produções com qualis publicados por anos da instituição
        ...
        """
        # query = """select t.ano, 
        #             sum(p.valor::int) filter (where p.key = 'A1') "A1",
        #             sum(p.valor::int) filter (where p.key = 'A2') "A2",
        #             sum(p.valor::int) filter (where p.key = 'A3') "A3",
        #             sum(p.valor::int) filter (where p.key = 'A4') "A4",
        #             sum(p.valor::int) filter (where p.key = 'B1') "B1",
        #             sum(p.valor::int) filter (where p.key = 'B2') "B2",
        #             sum(p.valor::int) filter (where p.key = 'B3') "B3",
        #             sum(p.valor::int) filter (where p.key = 'B4') "B4",
        #             sum(p.valor::int) filter (where p.key = 'C') "C"
        #             from programas_historico as t
        #             cross join jsonb_each_text(t.dados->'qualis') as p(key, valor) where t.id= %(id)s and t.ano>= %(anoi)s
        #             and t.ano<= %(anof)s
        ...
        for a in range(anoi, anof+1):
            ...
            for i in products:
                if a == i['ano']:
                    ano_in = True
            if ano_in == False:
                products.append({'ano': a, 'id_pessoa':'','A1': 0, 'A2': 0, 'A3': 0,
                                'A4': 0, 'B1': 0, 'B2': 0, 'B3': 0, 'B4': 0, 'C': 0})
        products = sorted(products, key=lambda k: k['ano'])
        ...
        for a in range(anoi, ultimo_ano_coleta+1):
            if a == ultimo_ano_coleta:
                lista_permanente = self.retorna_lista_permanente_do_ppg(id, a-1, db)
            else:
                lista_permanente = self.retorna_lista_permanente_do_ppg(id, a, db)
            permanentes[a] = eval(lista_permanente)

        #if anof < ano_atual:
        products_atual = self.retorna_quantidade_artigos_periodicos_publicados(
            id, ultimo_ano_coleta, db)

        if products_atual and len(products_atual) > 0:
            products.extend(products_atual)
            products = sorted(products, key=lambda k: k['ano'])

        return {'produtos': products, 'permanentes':permanentes}
      
    def retorna_lista_permanente_do_ppg(self, id: str, ano: int, db: DBConnector):
        """
        Retorna o número de professores permanentes do PPG
        """
        query = """select dados->>'permanentes' as permanentes from programas_historico where 
                ano = %(ano)s and id_programa = (select id_programa from programas where codigo_programa = %(id)s)"""
        row = db.fetch_one(query, id=id, ano=ano)
    ...
    def retorna_quantidade_artigos_periodicos_publicados(self, id: str, anof: int, db: DBConnector):
        """
        Retorna quantidade de artigos periódicos publicados no ano
        ...
        #         with query_ids as (
        #             with query_qualis as (
        #             select ld.id_sucupira, 
        #                 issn, 
        #                 titulo, 
        #                 (select qualis from capes_qualis_periodicos as cqp 
        #                         where cqp.issn = artigos_total.issn and 
        #                         cqp.area ilike  ANY (
		# 	 						select '%%' || nm_area_avaliacao || '%%' from programas where id = %(id)s )
        #                              group by qualis
        #                 ) as qualis, 
        #                 ano, 
        #                 count(*) as quantidade 
        ...
        #                         where lpb.tipo = 'ARTIGO-PUBLICADO' and 
        #                             d."DETALHAMENTO-DO-ARTIGO"->>'@ISSN' != '' and 									  
        #                             cast(d."DADOS-BASICOS-DO-ARTIGO"->>'@ANO-DO-ARTIGO' as integer) = %(anof)s 
        #                     ) as artigos_total
        #                 inner join lattes_docentes as ld on (ld.num_identificador = artigos_total.num_identificador and ld.id_sucupira != '')
        #                 group by ld.id_sucupira, issn, titulo, qualis, ano
        #             )
        ...
        #         select qids.ano, qids.issn, qids.titulo, qids.qualis, qids.ids, qids.quantidade from query_ids as qids
        #             inner join docentes_programas as dp on ids like '%%'||dp.id_docente||'%%' and dp.id_programa = %(id)s
        #             group by qids.ano, qids.issn, qids.titulo, qids.qualis, qids.ids, qids.quantidade
        #     )
        #     select  qq.ano,
        #             SUM(CASE WHEN qualis = 'A1' THEN 1 ELSE 0 END) AS "A1",
        #                 SUM(CASE WHEN qualis = 'A2' THEN 1 ELSE 0 END) AS "A2",
        ...
        row = db.fetch_all(query, id=id, anof=anof, anocoleta=anof-1)
        products_atual = [dict(r) for r in row]
        return products_atual
    ...
    def retorna_tempos_de_conclusao(self, id: str, anoi: int, anof: int, db: DBConnector):
        ...
        Paramêtros:
            ...
            db(class): DataBase
        """
        # query = """select nivel, situacao, 
        #         cast(EXTRACT(year FROM age(dt_situacao,dt_matricula))*12 + 
        #         EXTRACT(month FROM age(dt_situacao,dt_matricula)) as integer) as meses
        #         from discentes_programas where situacao = 'TITULADO' and 
        #         EXTRACT(year FROM age(dt_situacao,dt_matricula)) > 0 and 
        #         EXTRACT(year FROM age(dt_situacao,dt_matricula))*12 + 
        #         EXTRACT(month FROM age(dt_situacao,dt_matricula)) < 70 and
		# 		id_programa = %(id)s and ano >= %(anoi)s and ano <= %(anof)s"""
    ...
    async def retorna_orientandos_do_professor(self, id: str, id_docente: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        Paramêtros:
            ...
            db(class): DataBase
        """
        # query = """select distinct dis.nome, dp.ano, dp.nivel, cast(EXTRACT(year FROM age(dp.dt_situacao,dp.dt_matricula))*12 + 
        #                             EXTRACT(month FROM age(dp.dt_situacao,dp.dt_matricula)) as integer) as tempo, dp.situacao
        #             from docentes as d 
        #             inner join discentes_programas as dp on (d.nome = dp.nm_orientador)
        #             inner join discentes as dis on (dis.id = dp.id_discente)
        #             where dp.ano >= %(anoi)s and dp.ano <= %(anof)s and EXTRACT(month FROM age(dp.dt_situacao,dp.dt_matricula)) > 0 and 
        #             dp.id_programa = %(id)s and %(id_docente)s like '%%'||cast(d.id as text)||'%%'
        #             order by dp.ano"""
    ...
    async def getStatusOfProfessor(self, id: str, id_docente: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        # query = """SELECT STRING_AGG(categoria, ', ') AS categorias_concatenadas
        #             FROM docentes_programas AS dp
        #             INNER JOIN docentes AS d ON (dp.id_docente = d.id)
        #             WHERE id_programa = %(id)s AND %(id_docente)s like '%%'||cast(d.id as text)||'%%'
    ...
    async def retorna_lista_de_professores_atuais_para_ranking(self, id: str, db: DBConnector):
        """
        ...
        professores = self.retorna_lista_docentes_do_ppg_do_ultimo_ano(id, db)
        ...
        for p in professores:
            if 'nome' in p:
                ...
                datalattes[id_professor] = await self.retorna_tempo_de_atualizacao_do_lattes_do_professor(id_professor, db)
                # FONTE: lattes
                producoes[id_professor] = await self.retorna_producoes_do_professor_totais(id_professor,db)
                
                # query = f"select cast(id as text) from docentes where nome ilike '{id_professor}' limit 1"
                if id_professor:
                    avatares[id_professor] = await self.retorna_link_avatar_lattes(id_professor, True, db)

        return {'professores': professores,'produtos': producoes, 'avatares': avatares, 'datalattes': datalattes}
    
  
    async def retorna_lista_de_professores_por_ano(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        Retorna lista de professores por ano
    ...
    left join query_ids as qids on ids like '%%'||p.lattes||'%%'
        ...
        for p in professors:
            if 'nome' in p:
                ...
                datalattes[id_professor] = await self.retorna_tempo_de_atualizacao_do_lattes_do_professor(id_professor, db)
                # FONTE: lattes
                producoes[id_professor] = await self.retorna_producoes_do_professor(id_professor, anoi, anof, db)
                # FONTE: sucupira
                orientados[id_professor] = await self.retorna_orientandos_do_professor(id, p['id_sucupira'], anoi, anof, db)
                # FONTE: sucupira
                status[id_professor] = await self.getStatusOfProfessor(id, p['id_sucupira'], anoi, anof, db)
        ...
        if len(medias['pais']) > 0:
            medias_uteis['PPGs do país'] = self.calcula_medias(medias['pais'])
        if len(medias['regiao']) > 0:
            medias_uteis['PPGs da região '+medias['nome_regiao']] = self.calcula_medias(medias['regiao'])
    ...
    async def retorna_link_avatar_lattes(self, ids: str, idlattes: bool, db: DBConnector):
        """
    ...
    def retorna_grafo_de_coautores_do_ppg(self, id: str, anoi: int, anof: int, db: DBConnector):
        ...
        Paramêtros:
            ...
            db(class): DataBase
        """
        # query = """SELECT p.titulo as id_producao, pa.nm_autor, pa.ano FROM public.producoes_autores as pa
        #             inner join producoes as p on (p.id = pa.id_producao)
        #             where pa.tp_autor ilike 'Docente' and pa.id_programa = %(id)s and p.subtipo ilike 'ARTIGO EM PER%%'
        #             and p.ano_publicacao>= %(anoi)s and p.ano_publicacao<= %(anof)s
        #             group by p.titulo, pa.nm_autor, pa.ano
        #             ORDER BY p.titulo ASC, pa.ano ASC"""
    ...
    def retorna_professores_por_categoria(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        Paramêtros:
            ...
            db(class): DataBase
        """
        # query = """select count(dp.categoria), dp.categoria, dp.ano 
        #             from docentes as d 
        #             inner join docentes_programas as dp on (d.id = dp.id_docente)
        #             where dp.ano >= %(anoi)s and dp.ano <= %(anof)s and 
        #             dp.id_programa = %(id)s
        #             group by dp.ano, dp.categoria order by dp.ano"""
    ...
    def retorna_quantidade_de_discentes_titulados(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        Paramêtros:
            ...
            db(class): DataBase
        """
        # query = """select count(*), ano, nivel from discentes_programas 
        #             where ano >= %(anoi)s and ano <= %(anof)s and id_programa = %(id)s and 
        #             situacao = 'TITULADO' group by ano, nivel order by ano"""
    ...
    async def retorna_producoes_do_professor_totais(self, id: str, db: DBConnector):
        """
    ...
    def retorna_dados_de_projetos(self, id: str, anoi: int, anof: int, db: DBConnector):
        ...
        #                         select num_identificador from lattes_docentes 
        #                         where id_sucupira like ANY (
        #                             select '%%' || id_docente || '%%' from docentes_programas where id_programa = %(id)s
        #                             )
    ...
    def retorna_dados_de_linhas_de_pesquisa(self, id: str, anoi: int, anof: int, db: DBConnector):
        ...
        # query2 = """
        #     select subtipo as source, CASE WHEN linha_pesquisa IS NULL OR linha_pesquisa = 'null' OR (linha_pesquisa = '') THEN 'NÃO INFORMADO' ELSE linha_pesquisa END as target, count(*) as value from producoes 
        #     where id_programa = %(id)s and ano_publicacao >= %(anoi)s and ano_publicacao <= %(anof)s
        #     group by subtipo, linha_pesquisa"""
    ...
    def retorna_dados_de_produtos_por_tcc(self, id: str, anoi: int, anof: int, db: DBConnector):
        # query1 = """
        #     select tp_producao as source, subtipo as target, count(*) as value from producoes 
        #     where id_programa = %(id)s and ano_publicacao >= %(anoi)s and ano_publicacao <= %(anof)s
        #     group by tp_producao, subtipo"""
        ...
        medias = self.retorna_media_dados_de_produtos_por_tcc_nacional(id, anos_validos[0], anos_validos[-1], db)
    ...
    def retorna_media_dados_de_produtos_por_tcc_nacional(self, id: str, anoi: int, anof: int, db: DBConnector):
        # query1 = """
        #     select tp_producao as source, subtipo as target, count(*) as value from producoes 
        #     where id_programa = %(id)s and ano_publicacao >= %(anoi)s and ano_publicacao <= %(anof)s
        #     group by tp_producao, subtipo"""
    ...
    def retorna_dados_de_projetos_e_linhas_de_pesquisa(self, id: str, anoi: int, anof: int, db: DBConnector):
        # query1 = """
        #     select projeto_pesquisa as source, linha_pesquisa as target, count(*) as value from producoes 
        #     where id_programa = %(id)s and ano_publicacao >= %(anoi)s and ano_publicacao <= %(anof)s and projeto_pesquisa != ''
        #     group by projeto_pesquisa, linha_pesquisa"""
    ...
    def retorna_indprod_medio_por_anos(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        Retorna o indprod médio do PPG por anos 
        ...
        query = """select avg(cast(dados->>'indProd' as float)) as indProdAll, pb.ano
                    from programas as p
                    inner join programas_historico as pb on (p.id_programa=pb.id_programa) where
                    p.situacao ilike '%%FUNCIONAMENTO%%' and 
					p.modalidade = (select modalidade from programas where codigo_programa = %(id)s) and
        ...
        dictindprods['maxima'] = nota_maxima
    ...
    def retorna_grafo_programas_de_coparticipacoes(self, id_ies: str, anoi: int, anof: int, db: DBConnector):
        programas = utils.retorna_programas_ppg(id_ies, db)

        #pessoas = {}
        grafo = {'nodes': [], 'edges': []}

        for programa in programas.keys():
            #if programa not in pessoas:
            #    pessoas[programa] = []
            grafo['nodes'].append({'id':programa, 'root':True, 'nome':programas[programa], 'importancia':3})
            
            query = f"""select distinct id_pessoa from coleta_docentes where id_programa = (select id_programa from programas where codigo_programa = '{programa}') and ano >= {anoi} and ano <= {anof}"""
            row = db.fetch_all(query)
            #pessoas[programa] = [r[0] for r in row]
            for r in row:
                grafo['nodes'].append({'id':r[0], 'cluster':programa, 'nome':r[0], 'importancia':3})
                grafo['edges'].append({'source': programa, 'target': r[0], 'value': 1})

        return grafo
    

    def retorna_grafo_programas_de_colaboracoes(self, id_ies: str, produto: str, anoi: int, anof: int, db: DBConnector):
        programas = utils.retorna_programas_ppg(id_ies, db)

        #pessoas = {}
        grafo = {'nodes': [], 'links': []}

        for e,programa in enumerate(programas.keys()):
            #if programa not in pessoas:
            #    pessoas[programa] = []
            grafo['nodes'].append({'id':programa, 'group':programas[programa], 'nome':programas[programa], 'importancia':3, 'tipo':'ppg'})
            
            query = f"""select distinct id_pessoa from coleta_docentes where id_programa = (select id_programa from programas where codigo_programa = '{programa}') and ano >= {anoi} and ano <= {anof}"""
            row = db.fetch_all(query)
            #pessoas[programa] = [r[0] for r in row]
            for r in row:
                grafo['nodes'].append({'id':r[0]+'_'+str(e), 'group':programas[programa], 'nome':r[0], 'importancia':3, 'tipo':'pessoa'})
                grafo['links'].append({'source': programa, 'target': r[0]+'_'+str(e), 'value': 1})
        ...
        # query = f"""SELECT pa.id_producao, pa.nm_autor, p.ano_publicacao, pa.id_programa FROM public.producoes_autores as pa
        #             inner join producoes as p on (p.id = pa.id_producao)
        #             {where} and p.ano_publicacao >= %(anoi)s and p.ano_publicacao <= %(anof)s
        #             group by pa.id_producao, pa.nm_autor, p.ano_publicacao, pa.id_programa
        #             ORDER BY p.ano_publicacao ASC"""
        
        query = f"""with q_prods as (
            select cp.nome_producao as id_producao, p.id_pessoa, prog.codigo_programa as id_programa from coleta_autores as ca
                    ...
                    where ca.ano >= %(anoi)s and ca.ano <= %(anof)s
                        and ca.tipo_vinculo = 3
                        group by cp.nome_producao, p.id_pessoa,  prog.codigo_programa
                        )
						
            select id_producao, string_agg(id_programa, ',') as progs, string_agg(id_pessoa, ',') as pessoas from q_prods
            group by id_producao order by id_producao"""
        row = db.fetch_all(query, id_ies=id_ies, anoi=anoi, anof=anof, produto=produto)
        relacoes = [dict(r) for r in row]

        links_count = {}

        for rel in relacoes:
            pessoas_lista = set(rel['pessoas'].split(','))
            if len(pessoas_lista) > 1:
                prog_lista = list(set(rel['progs'].split(',')))
                if len(prog_lista) > 1:
                    comb = self.generates_combinations(prog_lista)
                    for c in comb:
                        if (c[0], c[1]) not in links_count:
                            links_count[(c[0], c[1])] = 1
                        else:
    ...
    def retorna_grafo_de_coautores_por_subtipo(self, id_ies: str, produto: str, tipo: str, anoi: int, anof: int, db: DBConnector):
        """
        ...
        print('tamanho do grafo:', len(grafo['nodes']))
        ...
        # for n in nodes_authors:
        #    grafo['nodes'].append({'id':str(n[0]), 'group':n[1]})
        return grafo,forca
    
    def retorna_grafo_de_coautores_do_programa(self, id_ies: str, id: str, anoi: int, anof: int, autor: str, db: DBConnector):
        """
        ...
        Paramêtros:
            ...
            db(class): DataBase
        """
        # query = """SELECT p.titulo as id_producao, pa.id_autor, pa.nm_autor, pa.ano FROM public.producoes_autores as pa
        #             inner join producoes as p on (p.id = pa.id_producao)
        #             where pa.tp_autor ilike 'Docente' and pa.id_programa = %(id)s and p.subtipo ilike 'ARTIGO EM PER%%'
        #             and p.ano_publicacao>= %(anoi)s and p.ano_publicacao<= %(anof)s
        #             group by p.titulo, pa.id_autor, pa.nm_autor, pa.ano
        #             ORDER BY p.titulo ASC, pa.ano ASC"""
    ...
    def retorna_grafo_de_coautores_por_subtipo_lattes(self, produto: str, anoi: int, anof: int, db: DBConnector):
        """
        Retorna um grafo de coautorias por subtipo lattes de todos os PPG's
        ...
        """
        tipo = ''
        query = "select * from lattes_producoes_duplicatas "
        produtos_in_db = {
            'ARTIGO EM PERIÓDICO': 'ARTIGO-PUBLICADO',
            'ARTES VISUAIS': 'ARTES-VISUAIS',
            'ARTIGO EM JORNAL OU REVISTA': 'TEXTO-EM-JORNAL-OU-REVISTA',
            'APRESENTAÇÃO DE TRABALHO': 'APRESENTACAO-DE-TRABALHO',
            'CARTAS, MAPAS OU SIMILARES': 'CARTA-MAPA-OU-SIMILAR',
            'CURSO DE CURTA DURAÇÃO': 'CURSO-DE-CURTA-DURACAO-MINISTRADO',
            'DESENVOLVIMENTO DE APLICATIVO': 'SOFTWARE',
            'DESENVOLVIMENTO DE MATERIAL DIDÁTICO E INSTRUCIONAL': 'DESENVOLVIMENTO-DE-MATERIAL-DIDATICO-OU-INSTRUCIONAL',
            'DESENVOLVIMENTO DE PRODUTO': 'PRODUTO-TECNOLOGICO',
            'EDITORIA': 'EDITORACAO',
            'MÚSICA': 'MUSICA',
            'ORGANIZAÇÃO DE EVENTO': 'ORGANIZACAO-DE-EVENTO',
            'OUTRA PRODUÇÃO CULTURAL': 'OUTRA-PRODUCAO-ARTISTICA-CULTURAL',
            'PATENTE': 'PATENTE',
            'PROGRAMA DE RÁDIO OU TV': 'PROGRAMA-DE-RADIO-OU-TV',
            'RELATÓRIO DE PESQUISA': 'RELATORIO-DE-PESQUISA',
            'TRABALHO EM ANAIS': 'TRABALHO-EM-EVENTOS',
        }
        if produto == 'LIVRO':
            query += "where tipo = 'CAPITULOS-DE-LIVROS-PUBLICADOS' or tipo = 'LIVROS-PUBLICADOS-OU-ORGANIZADOS' and ano >= %(anoi)s and ano <= %(anof)s order by ano"
        elif produto == 'DESENVOLVIMENTO DE TÉCNICA E SERVIÇOS TÉCNICOS':
            query += "where tipo = 'TRABALHO-TECNICO' or tipo = 'OUTRA-PRODUCAO-TECNICA' or tipo = 'PROCESSOS-OU-TECNICAS' and ano >= %(anoi)s and ano <= %(anof)s order by ano"
        else:
            query += f"where tipo = '{produtos_in_db[produto]}'"
            query += "and ano >= %(anoi)s and ano <= %(anof)s order by ano"

        row = db.fetch_all(query, anoi=anoi, anof=anof)
        coauts = [dict(r) for r in row]
        ...
        for c in coauts:
            if c['titulo'] not in artigos:
                artigos[c['titulo']] = []
            autores = c['autores'].split(",")
            for autor in autores:
                id_lattes = autor
                nome_autor = db.fetch_one(
                    f"select nm_completo from lattes_docentes where num_identificador = '{autor}'")
                if nome_autor:
                    autor = nome_autor[0]
                id_lattes_authors[autor] = id_lattes
                artigos[c['titulo']].append(autor)
                if autor not in nodes_authors:
                    nodes_authors[autor] = []
                    importance_authors[autor] = 1
        ...
        for k, v in nodes_authors.items():
            if k in id_lattes_authors:
                id_programa = self.getProgramByIDLattes(
                    id_lattes_authors[k], db)
                if id_programa:
                    importancia = 1
                    if k in importance_authors:
                        importancia = importance_authors[k]
                    grafo['nodes'].append(
                        {'id': k, 'group': id_programa, 'nome': programas_prp[id_programa], 'importancia': importancia})

        return grafo
    
    def getProgramByIDLattes(self, idlattes: str, db: DBConnector):
        """
        Retorna o id do Programa no qual o autor possui mais produções

        Paramêtros:
            idlattes(str): Idlattes do Professor
            db(class): DataBase
        """
        query = f"""
            SELECT id_programa FROM producoes_autores as pa
            WHERE cast(id_autor as text) = 
            ANY (
                select UNNEST(string_to_array(id_sucupira, ',')) AS split_array 
                from lattes_docentes where num_identificador = '{idlattes}'
            )
            GROUP BY id_programa
            ORDER BY COUNT(id_producao) DESC
            LIMIT 1;
        """
        row = db.fetch_one(query)
        if row is None:
            row = db.fetch_one(f"""select UNNEST(string_to_array(id_sucupira, ',')) AS split_array 
                from lattes_docentes where num_identificador = '{idlattes}'""")
            if row:
                row = db.fetch_one(
                    f"select id_programa from docentes_programas where id_docente = '{row[0]}'")
                if row:
                    return row[0]
            return None
        return row[0]
