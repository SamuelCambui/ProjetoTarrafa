...
app_filas= Celery('filas',
               broker=broker+':'+porta+'/1',  backend=broker+':'+porta+'/1',
               include=['backend.worker.despachante'])

app_filas.conf.update(
    task_routes={
        'backend.worker.despachante.*': {'queue': 'fila_despachante'}
    },
    
)

app_filas.conf.broker_connection_retry_on_startup = True