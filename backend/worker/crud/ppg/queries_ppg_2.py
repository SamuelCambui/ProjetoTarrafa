...
# from backend.core import utils, filtragem_lattes
...
class QueriesPPG():
    ...
    @tratamento_excecao_db_ppg()
    def retorna_metricas(self, id: str, anoi: int, anof: int, nome_metrica : str, db: DBConnector):
        """
        ...
        query = """select mbom, bom, regular, fraco from metricas_indicadores_areas_avaliacao
                    where nome_area_avaliacao = (select nome_area_avaliacao from programas where codigo_programa = %(id)s) 
                        and metrica = %(nome_metrica)s"""
        row = db.fetch_one(query, id=id, anof=anof, anoi=anoi, nome_metrica = nome_metrica)
        if row:
            indicadores = dict(row)
        else:
            indicadores = {'mbom':0, 'bom':0,'regular':0, 'fraco':0}
        
        query = """select cast(dados->>%(nome_metrica)s as float) as %(nome_metrica)s, ano from programas_historico where id_programa = (select id_programa from programas where codigo_programa = %(id)s)
					and (ano >= %(anoi)s and ano <= %(anof)s) order by ano"""
        row = db.fetch_all(query, id=id, anof=anof, anoi=anoi)
        metrica = [dict(r) for r in row]

        ano_in = False
        for a in range(anoi, anof+1):
            for i in metrica:
                if i[nome_metrica] is None:
                    i[nome_metrica] = 0.0
                if a == i['ano']:
                    ano_in = True
            if ano_in == False:
                metrica.append({nome_metrica: 0.0, 'ano': a})
        metrica = sorted(metrica, key=lambda k: k['ano'])
        
        return {nome_metrica: metrica, 'indicadores': indicadores}
    ...
    @tratamento_excecao_db_ppg()
    async def retorna_tempo_de_atualizacao_do_lattes_do_egresso(self, id: str, db: DBConnector):
        """
        Retorna a quantidade de tempo que o Perfi Lattes está desatualizado
    ...
    async def retorna_qualis_das_producoes(self, id: str, anoi: int, anof: int, db: DBConnector):
        ...
        #if anof < ano_atual:
        products_atual = self.retorna_quantidade_artigos_periodicos_publicados(id, ultimo_ano_coleta, db)
            ...
            for prod in prods:
                ...
                if prod['tipo'] not in contagens and 'ORIENTACOES' not in prod['tipo'] and 'APRESENTACAO-DE-TRABALHO' not in prod['tipo']:
                    ...
                    for dado in prod['dados']:
                        ...
                        # print(dado[dado_key][ano_key], type(dado[dado_key][ano_key]))
                        try:
    ...
    @tratamento_excecao_db_ppg()
    async def pegar_dados_egressos(self, id_ppg, ano_inicial, ano_final, db: DBConnector):
        query = f"""
        ...
        try:
            for egresso in egressos:
                ...
                datalattes[id_professor]['atualizacao_lattes'] = await self.retorna_tempo_de_atualizacao_do_lattes_do_egresso(id_professor, db)
    ...
    @tratamento_excecao_db_ppg()
    def retorna_tempo_de_atualizacao_do_lattes_egressos(self, id: str, anoi: int, anof: int, db: DBConnector):
        """
        Retorna a quantidade de Perfis Lattes desatualizados por PPG
    ...
    @tratamento_excecao_db_ppg()
    def retorna_quantidade_egressos_titulados_por_ano(self, id: str, anoi: int, anof: int, db: DBConnector):
    ...
    @tratamento_excecao_db_ppg()
    async def retorna_producoes_egresso(self, id: str, anoi:int, anof:int, db:DBConnector):
        ...
        for row in idlattes_egressos:
            ...
            datas[idlattes] = await self.retorna_producoes_do_professor(idlattes, anoi, ano_titulacao, db)
