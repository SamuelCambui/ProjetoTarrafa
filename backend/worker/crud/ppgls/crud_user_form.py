...
class CRUDUser():
    ...
    def retorna_dados_usuario_link_foto_lattes(self, email: str, db: DBConnectorGRADForm = None) -> tuple[Optional[UsuarioNoBanco], str]:
        ...
        """
        print(f"[DEBUG] Iniciando consulta para email: {email}")  # Verifica a entrada da função

        query = "SELECT * FROM usuarios WHERE email = %(email)s"
        row = db.fetch_one(query, email=email)

        if row:
            print(f"[DEBUG] Registro encontrado no banco: {row}")  # Verifica se encontrou o usuário
            ...
            try:
                user = UsuarioNoBanco(**dict_row)  # Corrigindo para usar apenas `dict_row`
                print(f"[DEBUG] Objeto UsuarioNoBanco criado com sucesso: {user}")

                # Obtendo o link do avatar
                rowavatar = crud.queries_ppg.retorna_link_avatar_lattes(user.idlattes, True) if user.idlattes else ""
                print(f"[DEBUG] Link do avatar retornado: {rowavatar}")

                return user, rowavatar
            except Exception as e:
                print(f"[ERRO] Falha ao criar UsuarioNoBanco: {e}")  # Captura erros ao instanciar o usuário
    ...
    def atualizar_usuario(self, usuario_atualizacao : UsuarioAtualizacao, db: DBConnectorGRADForm = None) -> tuple[bool, str]:
        ...
        """
        try:
            usuario_atualizacao.cpf = usuario_atualizacao.cpf if usuario_atualizacao.cpf else None
            usuario_atualizacao.curso = usuario_atualizacao.curso if usuario_atualizacao.curso else None
