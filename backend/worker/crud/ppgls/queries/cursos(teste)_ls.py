...
class QueriesCursosLS():
    ...
    #         SELECT historico."ano_letivo" AS ANO, alunos."sexo" AS SEXO, COUNT(DISTINCT alunos."matricula") AS QUANTIDADE
    #         FROM 
    #             alunos
    ...
    #     """
    #     query = """
    #         SELECT foo.necespecial, COUNT(foo.necespecial) AS quantidade 
    #         FROM (
    #             SELECT DISTINCT h.matricula_aluno, 
    #                 COALESCE(NULLIF(TRIM(al.necespecial), ''), 'NÃO TEM') AS necespecial
    #             FROM 
    #                 historico AS h
