...
class QueriesDisciplinas:
    ...
    def retorna_disciplina(
        ...
        query = """
            ...
            WHERE cod_disc = %(id)s and dis.id_ies = %(id_ies)s
        """
    ...
    def quantidade_prova_final(
        ...
        """
        Retorna a quantidade de provas finais feitas de uma disciplina por período.\n
        ...
        """
        query = """
            select count(*) as quantidade, ano_letivo, semestre from historico
            where cod_disc = %(id_disc)s
            and id_ies = %(id_ies)s
            and cast(ano_letivo as integer) between %(anoi)s and %(anof)s
            and prova_final is not null
            group by ano_letivo, semestre
    ...
    def histograma_notas_disciplina(
        ...
        """
        Retorna as classes e seus valores para montagem do histograma de uma disciplina.\n
        ...
        query = """
            with notas as (
                select nota from historico where cod_disc = %(id_disc)s and id_ies = %(id_ies)s
                and cast(ano_letivo as integer) between %(anoi)s and %(anof)s
                and nota is not null
            )
            select
    ...
    def boxplot_notas_prova_final(
        ...
        """
        Retorna as medidas do boxplot das provas finais de uma disciplina.\n
        ...
        query = """
            with query_notas as (
                select d.nome, prova_final,
                ntile(4) over (partition by d.nome order by prova_final) as quartil
                from historico as h 
                inner join disciplinas as d on d.cod_disc = h.cod_disc
                where h.cod_disc = %(id_disc)s and h.id_ies = %(id_ies)s
                and cast(h.ano_letivo as integer) between %(anoi)s and %(anof)s
            ...
            query_quartis as (
                select 
                    cast(max(case when quartil = 1 then prova_final end) as float) as primeiro_quartil,
                    cast(max(case when quartil = 2 then prova_final end) as float) as segundo_quartil,
                    cast(max(case when quartil = 3 then prova_final end) as float) as terceiro_quartil,
                    cast(round(stddev_pop(prova_final), 2) as float) as desvio_padrao,
                    cast(round(avg(prova_final), 2) as float) as media
                from query_notas
                having MAX(CASE WHEN quartil = 1 THEN prova_final END) IS NOT NULL
                AND MAX(CASE WHEN quartil = 2 THEN prova_final END) IS NOT NULL
                AND MAX(CASE WHEN quartil = 3 THEN prova_final END) IS NOT NULL
            )
            select *,
                GREATEST(primeiro_quartil - (1.5 * (terceiro_quartil - primeiro_quartil)), 0) as limite_inferior,
                LEAST(terceiro_quartil + (1.5 * (terceiro_quartil - primeiro_quartil)), 100) as limite_superior
        ...
        ret = db.fetch_all(
            query=query, id_disc=id_disc, anoi=anoi, anof=anof, id_ies=id_ies
        )
        return ret
