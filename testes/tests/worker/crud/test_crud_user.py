...
def test_get_user_db():
    user, avatar = crud.user.get_user_db(email='rene.veloso@unimontes.br')
...
def test_authenticate():
    user, useravatar = crud.user.authenticate(password='secret', email='rene.veloso@unimontes.br')     
    assert user.email == 'rene.veloso@unimontes.br'
    assert isinstance(user, UserInDB)
