...
function renderChartTimesConclusions(idCanvas, timesConclusions) {
  ...
  timesConclusions.forEach((item) => {
    ...
    if (nivel === "Mestrado Profissional") nivel = "Mestrado";
...
function renderChartStudentsGraduated(idCanvas, listStudents) {
  ...
  for (var i = 0; i < anos.length; i++) {
    if (groupedData["Mestrado"] && !groupedData["Mestrado"][anos[i]])
      groupedData["Mestrado"][anos[i]] = 0;
    if (groupedData["Doutorado"] && !groupedData["Doutorado"][anos[i]])
      groupedData["Doutorado"][anos[i]] = 0;
  }
...
function renderChartProfessorsByCategory(idCanvas, listProfessors) {
  ...
  for (var i = 0; i < anos.length; i++) {
    if (groupedData["VISITANTE"] && !groupedData["VISITANTE"][anos[i]])
      groupedData["VISITANTE"][anos[i]] = 0;
    if (groupedData["COLABORADOR"] && !groupedData["COLABORADOR"][anos[i]])
      groupedData["COLABORADOR"][anos[i]] = 0;
    if (!groupedData["PERMANENTE"][anos[i]])
      groupedData["PERMANENTE"][anos[i]] = 0;
  }
...
var listaArtigos;
function renderizarGraficoQualis(idCanvas, listQualisProductions, listArtigos) {
  listaArtigos = listArtigos;
  ...
  var max_qualis =
    ...
    listQualisProductions[anos[anos.length - 1]].A3 +
    listQualisProductions[anos[anos.length - 1]].A4 +
    listQualisProductions[anos[anos.length - 1]].B1 +
    listQualisProductions[anos[anos.length - 1]].B2 +
    listQualisProductions[anos[anos.length - 1]].B3 +
  ...
  canvas.addEventListener("click", function (event) {
    var activePoints = chartqualisproductions.getElementsAtEventForMode(
      event,
      "nearest",
      { intersect: true },
      true,
    ...
    if (activePoints.length > 0) {
      ...
      var clickedYear =
        chartqualisproductions.data.labels[activePoints[0].index];
      ...
      document.getElementById("listaArtigosModalLabel").innerText =
        `Total de artigos em ${clickedYear}: ${cont_artigos}`;
...
function renderChartQualis_discente(idCanvas, listQualisProductions) {
  ...
  if (chartqualisdiscenteproductions) {
    chartqualisdiscenteproductions.destroy();
  }
  chartqualisdiscenteproductions = new Chart(ctx, {
    type: "bar",
    data: {
      labels: anos,
      datasets: datasets_qualis,
...
function renderChartIndprods_(idCanvas, listAvgIndProds, indicadores) {
  ...
  for (
    var i = parseInt(listAvgIndProds["conceito"]);
  ...
  chartindprodsproductions = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        datalabels: {
          display: function (context) {
            if (context.dataset.label.includes("IndProd")) return true; // Mostra rótulos apenas para o conjunto de dados 'IndProd'
            return false;
          },
      ...
      scales: {
        ...
        y: {
          max: max_indprod + 0.2 * max_indprod,
          stacked: false,
          title: {
            display: true,
            text: "IndProd",
...
var chartindprodextsupsproductions;
function renderChartIndprodextsups_(idCanvas, listAvgIndProds, indicadores) {
  // Recalcular os anos, se necessário
  var anosSet = new Set(Object.keys(listAvgIndProds.indprods)); //map(data => data.ano));
  var anos = [...anosSet].sort();
  ...
  if (chartindprodextsupsproductions) {
    chartindprodextsupsproductions.destroy();
  }
  chartindprodextsupsproductions = new Chart(ctx, {
    type: "bar",
    data: {
      labels: anos,
      datasets: datasets_indprod,
...
function renderChartIndprods_simulacao(idCanvas, listAvgIndProds, indicadores) {
  // Recalcular os anos, se necessário
  var anosSet = new Set(Object.keys(listAvgIndProds.indprods)); //map(data => data.ano));
  var anos = [...anosSet].sort();
  ...
  if (indicadores.mbom === 0) annotation_values = {};
...
function renderChartIndCoautorias(
  idCanvas,
  listIndCoautorias,
...
var chartinddistori;
function renderChartIndDistOris(
  idCanvas,
  listIndDistOris,
  listAvgIndDistOris,
  indicadores,
...
let chartprofessorsproductions;
function renderChartProfessorProductions(produtos) {
...
function renderChartPositionIndProd(dados, id_ppg) {
  ...
  const dataset = {
    ...
    backgroundColor: values.map((value, index) => {
      if (dados.indprods[index].id === id_ppg) {
        return "#a1a1aa"; // Adjust the opacity here
      }
      return dados.indprods[index].status === "Pública Estadual"
        ? "#22d3ee"
        : dados.indprods[index].status === "Pública Federal"
          ? "#c084fc"
          : "#fbbf24";
    }),
  ...
  annotations_array["label_media"] = {
    ...
    //borderRadius: 2,
    content: [
      `Média notas ${dados.indprods[0].nota} = ${dados.media.toFixed(3)}`,
    ],
    font: {
      size: 14,
    },
    ...
    while (ncount < dados.media_maior.length) {
      ...
      if (dados.media_maior[ncount] !== null) {
        ...
        annotations_array["label_media_maior" + ncount] = {
          ...
          yValue: dados.media_maior[ncount] + 0.1,
          color: "#FF9289",
          //backgroundColor: 'rgba(245,245,245)',
          //borderColor: 'black',
          //borderWidth: 1,
          //borderRadius: 2,
          content: [
            `Média notas ${parseInt(dados.indprods[0].nota) + (ncount + 1)} = ${dados.media_maior[ncount].toFixed(3)}`,
          ],
          font: {
            size: 14,
          },
...
function index_renderChartProductions(idCanvas, listProductions) {
  const labels = listProductions.map((product) => {
    if (product.subtipo.length > 31) return product.subtipo.substring(0, 30);
...
function index_renderChartPeriodicProductions(
  idCanvas,
  listPeriodicProductions,
) {
...
let charttotalstechnicalproductions;
function index_renderChartTechnicalProductions(
  idCanvas,
  listTechnicalProductions,
) {
  const subtipos = [
    ...new Set(listTechnicalProductions.map((item) => item.subtipo)),
  ];
  const datasets = subtipos.map((subtipo) => ({
    label: subtipo,
    data: listTechnicalProductions
      .filter((item) => item.subtipo === subtipo)
      .map((item) => ({ x: item.ano, y: item.quantidade })),
    backgroundColor: obterCorAleatoria(),
  }));
...
function index_renderChartBiblioProductions(idCanvas, listBiblioProductions) {
  const subtipos = [
    ...new Set(listBiblioProductions.map((item) => item.subtipo)),
  ];
...
function index_renderChartStudentsLevel(idCanvas, listStudents) {
...
function renderizarGraficoAtualizacao(idCanvas, lattesdocs) {
...
var chartlogacessos;
function log_renderChartLogAcessos(idCanvas, listLogAcessos) {
  ...
  if (
    (type == "month" && year1 != year2) ||
    (type == "year" && year1 == year2)
  ) {
...
var chartloggrafos;
function log_renderChartLogGrafos(idCanvas, listLogGrafos) {
...
function extractAndSortProductTypes(data, excluded) {
  let allProductTypes = new Set(); // Create a set to store product types

  // Iterate over each year and extract product types
  Object.keys(data).forEach((year) => {
    Object.keys(data[year]).forEach((type) => {
      if (!excluded.includes(type)) allProductTypes.add(type); // Add to set, duplicates are automatically handled
    });
  });

  // Convert the set to an array and sort it alphabetically
  let sortedProductTypes = Array.from(allProductTypes).sort();

  // Return the sorted list of product types without duplicates
  return sortedProductTypes;
}

var charttccsproductions;
function renderChartTCCsProductions_(idCanvas, listtccsProductions) {
  ...
  if (charttccsproductions) {
    charttccsproductions.destroy();
  }
  charttccsproductions = new Chart(ctx, {
    type: "bar",
    data: {
      labels: Object.keys(listtccsProductions),
      datasets: datasets,
    },
    options: {
      maintainAspectRatio: false,
      plugins: {
        datalabels: {
          ...
          formatter: (value) => {
            if (value === 0) return "";
            else if (value !== null) return value.toFixed(2);
            return null;
          },
        },
      },
...
var charttccsproductions_tipos;
function renderChartTCCsProductions_tipos(
  idCanvas,
  listtccsProductions,
  medias,
) {
  ...
  var maior_valor = 0;
  var labels = extractAndSortProductTypes(listtccsProductions, ["Periódicos"]);
  var datasets = labels.map(function (label, index) {
    var dataEntries = Object.keys(listtccsProductions).map(function (year) {
      var value = listtccsProductions[year][label] || 0;
      if (maior_valor < value) maior_valor = value;
    ...
    return {
      label: label,
      backgroundColor: baseColors[index * Math.floor(53 / 10)],
      fill: false,
      spanGaps: true,
      skipNull: true,
      data: dataEntries,
  ...
  labels = extractAndSortProductTypes(medias, []);
  var datasets_medias = labels.map(function (label, index) {
    var dataEntries = Object.keys(medias).map(function (year) {
      var value = medias[year][label] || 0;
      if (maior_valor < value) maior_valor = value;
      return value === 0 ? null : value; // Return null for zero values to skip drawing these bars
    });
    ...
    return {
      label: label,
      backgroundColor: blackColors[index],
      fill: false,
      spanGaps: true,
      skipNull: true,
      data: dataEntries,
  ...
  if (charttccsproductions_tipos) {
    charttccsproductions_tipos.destroy();
  }
  charttccsproductions_tipos = new Chart(ctx, {
    type: "bar",
    data: {
      labels: Object.keys(listtccsProductions),
      datasets: datasets.concat(datasets_medias),
    },
    options: {
      maintainAspectRatio: false,
      plugins: {
      ...
      scales: {
        ...
        y: {
          ...
          stacked: false,
          beginAtZero: false,
          min: 0.01, // Set minimum scale value to 0.1 to help show bars with value of 1
          ticks: {
            callback: function (value, index, values) {
              if (
                value === 0.01 ||
...
var chartartigostccs;
function renderChartTCCsArtigos(idCanvas, listtccsProductions) {
  if (Object.keys(listtccsProductions).length == 0) return;

  var anosSet = new Set(Object.keys(listtccsProductions));
  var anos = [...anosSet].sort();
  ...
  for (var ano of anos) {
    a1.push(listtccsProductions[ano].A1);
    a2.push(listtccsProductions[ano].A2);
    a3.push(listtccsProductions[ano].A3);
    a4.push(listtccsProductions[ano].A4);
    b1.push(listtccsProductions[ano].B1);
    b2.push(listtccsProductions[ano].B2);
    b3.push(listtccsProductions[ano].B3);
    b4.push(listtccsProductions[ano].B4);
    c.push(listtccsProductions[ano].C);
  }

  //localização da legenda da coluna Lattes
  var max_qualis =
    listtccsProductions[anos[anos.length - 1]].A1 +
    listtccsProductions[anos[anos.length - 1]].A2 +
    listtccsProductions[anos[anos.length - 1]].A3 +
    listtccsProductions[anos[anos.length - 1]].A4 +
    listtccsProductions[anos[anos.length - 1]].B1 +
    listtccsProductions[anos[anos.length - 1]].B2 +
    listtccsProductions[anos[anos.length - 1]].B3 +
    listtccsProductions[anos[anos.length - 1]].B4 +
    listtccsProductions[anos[anos.length - 1]].C +
    5;
  ...
  if (chartartigostccs) {
    chartartigostccs.destroy();
  }
  chartartigostccs = new Chart(ctx, {
    type: "bar",
    data: {
      labels: anos,
      datasets: datasets_qualis,
...
const renderizarGraficoTCCsLivros = (idCanvas, producoesTCC) => {
  ...
  let maior_valor = Math.max(Object.values(producoesTCC));

  ctx.clearRect(0, 0, canvas.width, canvas.height);

  if (chartlivrostccs) {
    chartlivrostccs.destroy();
  }
  chartlivrostccs = new Chart(ctx, {
    type: "bar",
    data: {
      labels: Object.keys(producoesTCC),
      datasets: [
        {
          label: "Livros e capítulos de livros",
          backgroundColor: baseColors[0],
          data: Object.values(producoesTCC),
        },
      ],
    },
    options: {
...
criaGraficoPizza = (context, labels, data, chartLabel) => {
  new Chart(context, {
    type: "pie",
    data: {
      labels: labels,
