...
var listaArtigos;
function renderChartQualis_(idCanvas, listQualisProductions, listArtigos) {
    ...
    canvas.addEventListener('click', function (event){
        var activePoints = chartqualisproductions.getElementsAtEventForMode(event, 'nearest', {intersect: true }, true);

        if(activePoints.length > 0){
            var clickedDatasetIndex = activePoints[0].datasetIndex;
            var clickedYear = chartqualisproductions.data.labels[activePoints[0].index];
            ...
            document.getElementById('listaArtigosModalLabel').innerText = `Total de artigos em ${clickedYear}: ${cont_artigos}`;
    ...
    chartindprodsproductions = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    max: max_indprod+(0.20 * max_indprod),
                    stacked: false,
                    title: {
                        display: true,
                        text: 'IndProd',
...
var chartpartdis;
function renderChartPartDiss(idCanvas, listPartDiss, listAvgPartDiss, indicadores, nota_ppg) {
...
function renderChartIndCoautorias(idCanvas, listIndCoautorias, listAvgIndCoautorias, indicadores, nota_ppg) {
...
var chartindori;
function renderChartIndOris(idCanvas, listIndOris, listAvgIndOris, indicadores, nota_ppg) {
...
var chartinddistori;
function renderChartIndDistOris(idCanvas, listIndDistOris, listAvgIndDistOris, indicadores, nota_ppg) {
...
var chartindaut;
function renderChartIndAuts(idCanvas, listIndAuts, listAvgIndAuts, indicadores, nota_ppg) {
...
var chartinddis;
function renderChartIndDiss(idCanvas, listIndDiss, listAvgIndDiss, indicadores, nota_ppg) {
...
function index_renderChartPeriodicProductions(idCanvas, listPeriodicProductions) {
...
function index_renderChartTechnicalProductions(idCanvas, listTechnicalProductions) {
...
function renderChartLattesUpdate(idCanvas, lattesdocs) {
    ...
    Object.keys(data).forEach(year => {
        Object.keys(data[year]).forEach(type => {
            if (!excluded.includes(type))
                allProductTypes.add(type); // Add to set, duplicates are automatically handled
        });
    });
...
function renderChartTCCsProductions_(idCanvas, listtccsProductions) {
    ...
    // });
    var maior_valor = 0;
    var labels = extractAndSortProductTypes(listtccsProductions, ['total', 'BIBLIOGRÁFICA', 'TÉCNICA']);
...
function renderChartTCCsProductions_tipos(idCanvas, listtccsProductions, medias) {
    ...
    // canvas.addEventListener('click', function (event){
    //     var activePoints = charttccsproductions_tipos.getElementsAtEventForMode(event, 'nearest', {intersect: true }, true);

    //     if(activePoints.length > 0){
    //         var clickedDatasetIndex = activePoints[0].datasetIndex;
    //         var clickedYear = charttccsproductions_tipos.data.labels[activePoints[0].index];
...
function renderChartTCCsArtigos(idCanvas, listtccsProductions) {
    ...
    //localização da legenda da coluna Lattes
    var max_qualis = listtccsProductions[anos[anos.length-1]].A1+listtccsProductions[anos[anos.length-1]].A2+
                 listtccsProductions[anos[anos.length-1]].A3+listtccsProductions[anos[anos.length-1]].A4+
                 listtccsProductions[anos[anos.length-1]].B1+listtccsProductions[anos[anos.length-1]].B2+
                 listtccsProductions[anos[anos.length-1]].B3+listtccsProductions[anos[anos.length-1]].B4+
                 listtccsProductions[anos[anos.length-1]].C + 5;
...
var chartlivrostccs;
function renderChartTCCsLivros(idCanvas, listtccsProductions) {
    ...
    // canvas.addEventListener('click', function (event){
    //     var activePoints = chartlivrostccs.getElementsAtEventForMode(event, 'nearest', {intersect: true }, true);

    //     if(activePoints.length > 0){
    //         var clickedDatasetIndex = activePoints[0].datasetIndex;
    //         var clickedYear = chartlivrostccs.data.labels[activePoints[0].index];
    ...
    var maior_valor = Math.max(Object.values(listtccsProductions));
    ...
    chartlivrostccs = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: baseColors[0],
                data: Object.values(listtccsProductions)
            }]
        },
        options: {
            maintainAspectRatio: false,
